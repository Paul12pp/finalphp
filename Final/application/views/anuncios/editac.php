					...
					<div class="row">
						...
						<div class="col-md-8" style="background-color: ;">
							...
							<form method="post" action="<?php echo base_url('anuncio/guardarBici'); ?>" enctype="multipart/form-data">
								<div class="form-group">
									<input hidden readonly class="form-control" type="text" value="<?=$accesorio->id?>" name="id">
								</div>
								<div class="form-group">
									...
									<select class="form-control provincia" name="provincia" required="">
										...
										<?php 
											...
											for( $i=0; $i<count($option); $i++ ) {
											    if($accesorio->provincia==$option[$i]){
								...
								<div class="form-group">
									<label>Tel./cel (*)</label>
									<input class="form-control" type="text" value="<?=$accesorio->telefono?>" name="telefono" required>
								</div>
								<div class="form-group acceso">
									...
									<select class="form-control accesorio" name="tipo" required="">
										...
										<?php 
											$option = array('Bombas de aire','Bultos','Burros','Cámaras','Cascos','Chapas / Cleats','Computadoras','DVDs/Libros','Guantillas','Herramientas','Hidratación','Iluminación','Lentes','Limpieza/Lubricación','Nutrición','Otros','Primeros Auxilios','Protección','Relojes','Ropa','Seguridad','Termos/Termeras','Transporte/Racks','Zapatos');
											$output = '';
											for( $i=0; $i<count($option); $i++ ) {
											    if($accesorio->tipo==$option[$i]){
											     	echo "<option value='{$option[$i]}' selected>{$option[$i]}</option>";
											    }else{
											     	echo "<option value='{$option[$i]}'>{$option[$i]}</option>";
											    }
								...
								<div class="form-group">
									...
									<input class="form-control" type="text" value="<?=$accesorio->accesorio?>" name="accesorio" required>
								</div>
								<div class="form-group">
									<label>Marca(*)</label>
								...
								<div class="form-group">
									<label>Modelo</label>
									<input class="form-control" value="<?=$accesorio->modelo?>" type="text" name="modelo">
								</div>
								...
								<div class="form-group">
									<label>Precio (*)</label>
									<input class="form-control" value="<?=$accesorio->precio?>" type="text" name="precio" required>
								</div>
								<div class="form-group">
									<label>Titulo del anuncio (*)</label>
									<input class="form-control" type="text" value="<?=$accesorio->titulo?>" name="titulo" required>
								</div>
								<div class="form-group">
									<label>Descripcion (*)</label>
									<textarea class="form-control" name="descripcion" required><?=$accesorio->descripcion?></textarea>
								</div>
								...
								<div class="form-group">
									<input hidden class="form-control" type="text" value="<?=$accesorio->categoria?>" name="categoria" required>
								</div>
								<div class="form-group">
									<input hidden class="form-control" type="text" value="<?=$accesorio->fechaini?>" name="fechaini" required>
								</div>
								<div class="form-group">
									<input hidden class="form-control" type="text" value="<?=$accesorio->fechafin?>" name="fechafin" required>
								</div>
