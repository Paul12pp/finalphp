			...
			<div class="col-md-8">
				<ul class="nav nav-tabs" id="myTab" role="tablist">
				  <li class="nav-item active">
				    <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true" onclick="openCity(event, 'home')">Listado de accesorios (<?=$total?>)</a>
				  </li>
				</ul>
				  <div class="tab-content" id="myTabContent">
				  <div class="tab-pane active" id="home">
					<?php
				  		foreach ($accesorios as $acceso) {
				  			$ver = base_url("/anuncio/detalleac/?id={$acceso->id}");
				  			$link = base_url();
				  			$descr = substr($acceso->descripcion,0, 260);
				  			$fecha = $acceso->fechaini;
							$d2 = strtotime('+0 days');
									...
									<div class='card-body'>
										<div class='row'>
											<div class='col-md-auto'>
												<a href='{$ver}'><img class='car-img-top' src='{$link}mthumb.php?src={$link}{$acceso->imagen}&w=150&h=150' alt='Card image cap'></a>
											</div>
											<div class='col'>
												<div class='row'>
													<div class='col-8 align-self-start'>
														<h5 class='card-title'><b>{$acceso->titulo}</b></h5>
														<div class='row'>
															<div class='col-md-auto'>
																<h6 class='card-subtitle mb-2 text-muted'>{$acceso->tipo}</h6>
															</div>
															<div class='col'>
																<h6 class='card-subtitle mb-2 text-muted'>{$acceso->username}</h6>
															</div>
															<div class='col'>
													...
													<div class='col align-self-end'>
														<h4>{$acceso->moneda}{$acceso->precio}</h4>
													</div>
												</div>
				  	...
				  	?>
					  	<div class="" style="text-align: right; ">	
							<?php
							$link = base_url('anuncio/listaccesorios/');
							if($totalpages>0){
								$pp = 'Next';
								$ppi = 'Previous';
