					...
					<div class="row">
						...
						<div class="col-md-8" style="background-color: ;">
							...
							<form method="post" action="<?php echo base_url('anuncio/guardarBici'); ?>" enctype="multipart/form-data">
								<div class="form-group">
									<input hidden readonly class="form-control" type="text" value="<?=$servicio->id?>" name="id">
								</div>
								<div class="form-group">
									...
									<select class="form-control provincia" name="provincia" required="">
										...
										<?php 
											...
											for( $i=0; $i<count($option); $i++ ) {
											    if($servicio->provincia==$option[$i]){
								...
								<div class="form-group">
									<label>Tel./cel (*)</label>
									<input class="form-control" type="text" value="<?=$servicio->telefono?>" name="telefono" required>
								</div>
								<div class="form-group servi">
									...
									<select class="form-control servicio" name="tipo" required="">
										...
										<?php 
											$option = array('Clases','Eventos','Excursiones','Fitting','Fotografia y video', 'Mantenimiento','Personalización','Reparación','Varios');
											$output = '';
											for( $i=0; $i<count($option); $i++ ) {
											    if($servicio->tipo==$option[$i]){
											     	echo "<option value='{$option[$i]}' selected>{$option[$i]}</option>";
											    }else{
											     	echo "<option value='{$option[$i]}'>{$option[$i]}</option>";
											    }
								...
								<div class="form-group">
									<label>Precio (*)</label>
									<input class="form-control" value="<?=$servicio->precio?>" type="text" name="precio" required>
								</div>
								<div class="form-group">
									<label>Titulo del anuncio (*)</label>
									<input class="form-control" type="text" value="<?=$servicio->titulo?>" name="titulo" required>
								</div>
								<div class="form-group">
									<label>Descripcion (*)</label>
									<textarea class="form-control" name="descripcion" required><?=$servicio->descripcion?></textarea>
								</div>
								...
								<div class="form-group">
									<input hidden class="form-control" type="text" value="<?=$servicio->categoria?>" name="categoria" required>
								</div>
								<div class="form-group">
									<input hidden class="form-control" type="text" value="<?=$servicio->fechaini?>" name="fechaini" required>
								</div>
								<div class="form-group">
									<input hidden class="form-control" type="text" value="<?=$servicio->fechafin?>" name="fechafin" required>
								</div>
